...
echo Starting TerminalTanks Revert Simulation...
...
echo Applying simulated revert tweaks...

rem Apply simulated revert tweaks (no prompts)
        ...
        if exist "%%D\Revert\" (
            ...
            for %%F in ("%%D\Revert\*.reg" "%%D\Revert\*.cmd") do (
                ...
                if /i "%%~xF"==".reg" (
                    ...
                    echo [!PROCESSED_FILES!/!TOTAL_FILES!] Simulated success: !FILE_NAME!
                ) else (
...
echo Simulation complete! No changes applied.
pause
exit /b